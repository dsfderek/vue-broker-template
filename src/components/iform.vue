<template>
  <div class="iform">
    ...
    <el-form :model='iformData' :validate-on-rule-change='false' :rules="!disabled ? iRules : {}" :ref="formName"   :inline='inline' :disabled='disabled' v-update='formName' class="iform-container">
      <div 
      v-for='(item, index) in iformModel'  
      :key='index'
      class="iform-item-flex"
      >
...
export default {
  ...
  methods: {
    ...
    initForm (list) {
       ...
       */
      this.iformData = utils.clearObj(Object.assign({}, formData, this.formData))
      /*
       * 设置表单渲染的模型
       */
